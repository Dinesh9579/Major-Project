        ...
        for subject in subjects:
            ...
            for i in range(1, len(self.pts)):
                if self.pts[i] > self.pts[i - 1]:
                    ...
                    if self.pts[i] > 30 and self.pts[i] < 55:
                        print("Eyes have been closed for 30-55 frames! - Print '-'")
                        log("Eyes have been closed for 30-55 frames! Print '-' ")
                        self.L.append("-")
                        self.pts = deque(maxlen=512)
                        break
                    # elif self.pts[i] > 15 and self.pts[i] < 30:
                    elif self.pts[i] > 15 and self.pts[i] < 30:
                        print("Eyes have been closed for 15-30 frames! Print '.' ")
                        log("Eyes have been closed for 15-30 frames! - Print '.'")
                        self.L.append(".")
                        self.pts = deque(maxlen=512)
                        break
                    ...
                    # elif self.pts[i] > 60:
                        ...
                        print("Eyes have been closed for 60 frames! Remove last morse character")
                        log("Eyes have been closed for 60 frames! - Remove last morse character")
                        if len(self.L)>0:
                            self.L.pop()
                        else:
